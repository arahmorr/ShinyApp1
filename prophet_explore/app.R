...
ui <- dashboardPage(
  dashboardHeader(title = "Prophet Forecast"),
  
  ## Sidebar ------------------------------------
  dashboardSidebar(
    sidebarMenu(
            menuItem("Prophet Forecast", tabName = "Prophet")
    )
  ),
              ...
              fluidRow(
                box(width = 12,
                    tabsetPanel(id = "inTabset",
                                ## TAB 1 : Upload Data --------------------------
                                tabPanel(title = "Cargar Data", value = "panel1",
                                         
                                         fluidRow(
                                           ## upload main dataset -----------------
                                           column(width = 6,
                                                  # column(width = 12,
                                                  tags$h4("Base de datos"),
                                                  helpText("Debe contener por lo menos 2 columnas (ds, y)."),
                                                  fileInput("ts_file","Cargar archivo CSV",
                                                            accept = c(
                                                              "text/csv",
                                                  ...
                                                  conditionalPanel(condition = 'output.panelStatus',
                                                                   helpText("Estas son las primeras 6 filas de la base de datos:")),
                                           ...
                                           ## upload holidays -----------------
                                           column(width = 6,
                                                  tags$h4("Vacaciones y días festivos (opcional)"),
                                                  helpText("Debe contener por lo menos 2 columnas (ds, y)."),
                                                  fileInput("holidays_file","Cargar archivo CSV",
                                                            accept = c(
                                                              "text/csv",
                                                              "text/comma-separated-values,text/plain",
                                                              ".csv")),
                                         ...
                                         fluidRow(
                                           column(width = 2, offset = 10,
                                                  shinyjs::disabled(actionButton("next1", "Continuar",
                                                                                 style = "width:100%; font-size:200%"))))
                                ),
                                ## TAB 2 : Set Parameters -----------------------------------
                                tabPanel(title = "Ajuste de parámetros", value = "panel2",
                                         fluidRow(
                                           column(width = 8,
                                                  column(width = 8, offset = 2,
                                                         tags$h3("Parámetros Prophet")),
                                                  column(width = 6,
                                                         
                                                         radioButtons("growth","Crecimiento",
                                                                      c('linear','logistic'), inline = TRUE),
                                                         
                                                         ### parameter: yearly.seasonality
                                                         checkboxInput("yearly","yearly.seasonality", value = TRUE),
                                                         
                                                         radioButtons("addormul", "Tipo de estacionalidad",choices = list("Aditiva" = 'additive', "Multiplicativa" = 'multiplicative'), 
                                                                                 selected = 'additive'),
                                                         
                                                         ### parameter: weekly.seasonality 
                                                         checkboxInput("monthly","weekly.seasonality", value = TRUE),
                                                         ### parameter: n.changepoints
                                                         numericInput("n.changepoints","Número de puntos de cambio: n.changepoints", value = 25),
                                                      
                                                         ### parameter: seasonality.prior.scale
                                                         numericInput("seasonality_scale","Magnitud de estacionalidad: seasonality.prior.scale", value = 10),
                                                         
                                                         ### parameter: changepoint.prior.scale
                                                         numericInput("changepoint_scale","Flexibilidad: changepoint.prior.scale", value = 0.05, step = 0.01)),
                                                  column(width = 6,
                                                         
                                                         ### parameter: holidays.prior.scale
                                                         numericInput("holidays_scale","Magnitud de días festivos: holidays.prior.scale", value = 10),
                                                         
                                                         ### parameter: mcmc.samples
                                                         numericInput("mcmc.samples", "Incertidumbre en la estacionalidad: mcmc.samples", value = 0),
                                                         
                                                         ### parameter: interval.width
                                                         numericInput("interval.width", "Intervalos de incertidumbre: interval.width", value= 0.8, step = 0.1))
                                                  
                                           ),
                                           ## predict parameters --------------------
                                           column(width = 4,
                                                  tags$h3("Parámetros de predicción"),
                                                  ### paramater: periods
                                                  numericInput("periods","Números de períodos",value=365),
                                                  ### parameter: freq
                                                  selectInput("freq","Frecuencia",
                                         ...
                                         fluidRow(
                                           column(width = 2, 
                                                  actionButton("back2", "Regresar",
                                                               style = "width:100%; font-size:200%")),
                                           column(width = 2, offset = 8,
                                                  actionButton("next2", "Continuar",
                                                               style = "width:100%; font-size:200%"))
                                         )
                                ),
                                ## TAB 3 : Fit Propher Model ----------------------
                                tabPanel(title = "Ajuste del Modelo", value = "panel3", 
                                         fluidRow(
                                           # box(width = 12, 
                                           column(width = 12,
                                                  shinyjs::disabled(actionButton("plot_btn2", "Ajuste del modelo Prophet",
...
server <- function(input, output, session) {
  ...
  prophet_model <- eventReactive(input$plot_btn2,{
    ...
    kk <- prophet(datx,seasonality.mode=input$addormul,
                  growth = input$growth,
